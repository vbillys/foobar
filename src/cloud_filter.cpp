 ...
 *  - firmware problem (SOLVED: using qsort)
 *  - noises, z constraint may be? -> can use prediction kah?
 *  - -> may be try to use multiple tau values... or RAN SAC
 *  - obstacle detection (both positive and negative z) not implemented
...
VPointCloud::Ptr g_outMsg_debug2(new VPointCloud());
...
CurbRing cloudProcess ( const VPointCloud::Ptr& outMsg_debug , int ring_no, std::vector<ZVCand> &cand_accum_z_right, std::vector<ZVCand> &cand_accum_z_left);
...
#define Z_CUT_LIMIT 0.5
    ...
    VPointCloud::Ptr outMsg_curb2(new VPointCloud());
    outMsg_curb2->header.stamp = pcl_conversions::toPCL(cloud_msg->header).stamp;
    outMsg_curb2->header.frame_id = cloud->header.frame_id;
    outMsg_curb2->height = 1;
    ...
    g_outMsg_debug2->header.stamp = pcl_conversions::toPCL(cloud_msg->header).stamp;
    g_outMsg_debug2->header.frame_id = cloud->header.frame_id;
    g_outMsg_debug2->clear();
    g_outMsg_debug2->height = 1;
        ...
        )
    {
	  ...
	  std::vector<ZVCand> _zvcand_right, _zvcand_left;
      CurbRing _result = cloudProcess ( outMsg_debug , _ring_no_being_processed , _zvcand_right, _zvcand_left);
      if (!_result.data_invalid)
      {
		  for (int j = 0; j < _zvcand_right.size(); j++){
			  velodyne_pointcloud::PointXYZIR _point_new;
			  _point_new.x = _zvcand_right[j].x;
			  _point_new.y = _zvcand_right[j].y;
			  _point_new.z = _zvcand_right[j].z;
			  _point_new.ring = 0;
			  _point_new.intensity = 0;
			  outMsg_curb2->push_back(_point_new);
		  }
		  for (int j = 0; j < _zvcand_left.size() ; j++){
			  velodyne_pointcloud::PointXYZIR _point_new;
			  _point_new.x = _zvcand_left[j].x;
			  _point_new.y = _zvcand_left[j].y;
			  _point_new.z = _zvcand_left[j].z;
			  _point_new.ring = 0;
			  _point_new.intensity = 0;
			  outMsg_curb2->push_back(_point_new);
		  }
		  if (_result.got_left)
		  {
			  ...
			  fprintf(fp,"%d\t%d\t%.3f\t%.3f\n"
		       ...
		       debug_print_0("found left  curb at ring %d ...ind=%d\n", _ring_no_being_processed,_result.left_ind);
		  }
		  else
		  {
		  ...
		  if (_result.got_right)
		  {
			  ...
			  _point_new.intensity = 0;
			  outMsg_curb->push_back(_point_new);
			  debug_print_0("found right curb at ring %d ...ind=%d\n", _ring_no_being_processed,_result.right_ind);
		  }
		  else
		  {
...
{
  //IndexedAngle* _a = (IndexedAngle*)a;
...
CurbRing cloudProcess ( const VPointCloud::Ptr& outMsg_debug , int ring_no, std::vector<ZVCand> &cand_accum_z_right, std::vector<ZVCand> &cand_accum_z_left)
{
    ...
    debug_print_6("data ring no %d\n", ring_no);
    float _prev_angle = g_angle_data_ring[ring_no][min_index];
    bool _sort_required = false;
    IndexedAngle angle_sorted[outMsg->points.size()];
    float min_height = outMsg->points[0].z;
    int min_height_index = 0;
    int min_height_index_t = min_index;
    for (int next = 0; next < outMsg->points.size(); next++)
    {
		...
		if (_prev_angle < _angle)
		{
		    debug_print_6("%d : %.3f INCONSISTENT!!!\n", next, _angle);
		    _sort_required = true;
	    }
	    else
	    {
		...
		if (min_height > _point_new.z) {
			min_height_index_t = _index_reindex;
			min_height_index = next;
			min_height = _point_new.z;
		}
	...
	if (_sort_required)
	{
		...
		for (int next = 0; next < outMsg->points.size(); next++)
        {
			...
			float _angle  = g_angle_data_ring[ring_no][_index_reindex];
			//float _angle  =  angle_sorted[next].angle;
	...
	else
	{
        ...
        {
			velodyne_pointcloud::PointXYZIR _point_new;
		    _point_new.ring = cloud_org->points[next].ring;
		    _point_new.x    = cloud_org->points[next].x;
		    _point_new.y    = cloud_org->points[next].y;
		    _point_new.z    = cloud_org->points[next].z;
		    _point_new.intensity = next;
		    
		    g_outMsg_debug2->push_back(_point_new);
		}
		//g_outMsg_debug2 = cloud_org;
	}
...
#define CURB_ZCROSS_THRESHOLD 0.02//0.005 //0.02
	...
	for (int j = 0; j < cloud_org->points.size(); j++)
    {
		...
		//filter_data[j]   = //2*radial_data[j]
		                   ...
		                   //fabs(radial_data[t_bef]-radial_data[t_after])
		//                  ;
		float _sum_z = 0;int _cnt_avg = 0;
		for (int i = t_bef; i <= t_after; i++)
		{
			_sum_z += cloud_org->points[i].z;
			_cnt_avg++;
		}
		float _avg_z = _sum_z / _cnt_avg;//(t_after - t_bef + 1);
		...
		if (filter_data[j] > 0)// CURB_ZCROSS_THRESHOLD)
		{
		...
		else if (filter_data[j] <0)// -CURB_ZCROSS_THRESHOLD)
		{
			cloud_org->points[j].intensity = -1.;
			//debug_print_3("%d : %.3f %.3f %.3f %.3f \n", j, filter_data[j], cloud_org->points[j].z, cloud_org->points[j].y, g_angle_data_ring_org [ring_no][j]);
		}
		...
		//if (cloud_org->points[j].z > min_height + 0.1)
		//{
			//cloud_org->points[j].intensity =  1.;
			////debug_print_3("%d : %.3f NON ROAD\n", j, cloud_org->points[j].z);
		//}
		//else if (cloud_org->points[j].z < min_height + 0.05)
		//{
			//cloud_org->points[j].intensity = -1.;
			////debug_print_3("%d : %.3f ROAD\n", j, cloud_org->points[j].z);
		//}
		//else
		//{
			//cloud_org->points[j].intensity = 0.;
			////debug_print_3("%d : %.3f UNKNOWN\n", j, cloud_org->points[j].z);
		//}
		//cloud_org->points[j].z = _avg_z;
		lowpass_zdata[j] = _avg_z;
	...
	float lowpass_filter_data[cloud_org->points.size()];
	tau = 5;
	for (int j = 0; j < cloud_org->points.size(); j++)
    {
		t_bef   = j - tau; if (t_bef < 0) t_bef = 0;
		t_after = j + tau; if (t_after > cloud_org->points.size()-1) t_after = cloud_org->points.size()-1;
		float _sum_f = 0;int _cnt_avg = 0;
		for (int i = t_bef; i <= t_after; i++)
		{
			_sum_f += filter_data[i];
			_cnt_avg++;
		}
		//float _avg_f = _sum_f / _cnt_avg;
		lowpass_filter_data[j] = _sum_f / _cnt_avg;
	}
	
	//float zfilter_data[cloud_org->points.size()] = {0};
	//zfilter_data[half_index] = 0;
	std::vector<float> zfilter_data;
	zfilter_data.assign(cloud_org->points.size(),0.);
	
	debug_print_3("\n");
	int _last_zindex = half_index;
	float _accum_z = 0;
	float _last_zv = cloud_org->points[half_index].z;
	float _prev_zv = _last_zv;
	int   _prev_zv_index = half_index;
	float max_accum_z_right = 0;
	int   indexof_max_accum_z_right = half_index;
	//std::vector<ZVCand> cand_accum_z_right;
	for (int j = half_index+1; j < cloud_org->points.size(); j++)
    {
	     float _curr_zv = cloud_org->points[j].z;
	     if (_curr_zv >= _prev_zv)
	     {
			 _accum_z = _curr_zv - _last_zv;
			 
		 }
		 //else if (lowpass_zdata[j] >= lowpass_zdata[_prev_zv_index])
		 //else if (lowpass_zdata[j] > _last_zv + 0.005*(j - _last_zindex))
		 //{
			 
		 //}
		 else
		 {
			 if (max_accum_z_right < _accum_z) {
				 max_accum_z_right = _accum_z;
				 indexof_max_accum_z_right = _last_zindex;
			 }
			 if (_accum_z >= 0.08) {
				 ZVCand _taz;
				 _taz.z = _accum_z;
				 _taz.y = cloud_org->points[_last_zindex].y;
				 _taz.x = cloud_org->points[_last_zindex].x;
				 _taz.index = _last_zindex;
				 cand_accum_z_right.push_back(_taz);
			 }
			 zfilter_data[_last_zindex] = _accum_z;
			 _last_zindex = j;
			 _last_zv = _curr_zv;
			 _accum_z   = 0;
		 }
	...
	debug_print_3("\n");
	_last_zindex = half_index;
	_accum_z = 0;
	_last_zv = cloud_org->points[half_index].z;
	_prev_zv = _last_zv;
	_prev_zv_index = half_index;
	float max_accum_z_left = 0;
	int   indexof_max_accum_z_left = half_index;
	//std::vector<ZVCand> cand_accum_z_left;
	for (int j = half_index-1; j >= 0; j--)
    {
		 ...
		 {
			 if (max_accum_z_left < _accum_z){ 
				 max_accum_z_left = _accum_z;
				 indexof_max_accum_z_left = _last_zindex;
			 }
			 if (_accum_z >= 0.08) {
				 ZVCand _taz;
				 _taz.z = _accum_z;
				 ...
				 _taz.x = cloud_org->points[_last_zindex].x;
				 _taz.index = _last_zindex;
				 cand_accum_z_left.push_back(_taz);
			 }
			 zfilter_data[_last_zindex] = _accum_z;
			 _last_zindex = j;
			 _last_zv = _curr_zv;
	...
	debug_print_3("\n");
	debug_print_3("ZFilter data:\n");
	for (int j = 0; j < cloud_org->points.size(); j++)
    {
		debug_print_3("%d : %.3f %.3f %.3f %.3f\n", j, zfilter_data[j], cloud_org->points[j].z, cloud_org->points[j].y, lowpass_zdata[j]);
	}
	
	debug_print_3("\n");
	debug_print_3("maxzacc l: %.3f at %d , r: %.3f at %d\n"
	              , max_accum_z_left
	              , indexof_max_accum_z_left 
	              , max_accum_z_right
	              , indexof_max_accum_z_right
	             );
	debug_print_3("left  details...:\n");
	for (int j =0; j < cand_accum_z_left.size(); j++){
		debug_print_3("%.3f at %d %.3f \n"
		              , cand_accum_z_left[j].z
		              , cand_accum_z_left[j].index
		              , cand_accum_z_left[j].y
		             );
	}
	debug_print_3("right details...:\n");
	for (int j =0; j < cand_accum_z_right.size(); j++){
		debug_print_3("%.3f at %d %.3f \n"
		              , cand_accum_z_right[j].z
		              , cand_accum_z_right[j].index
		              , cand_accum_z_right[j].y
		             );
	}
	debug_print_3("\n");
...
#define CURB_YDIFF_THRESHOLD 2.//1.5//0.5
	...
	else
	{
		...
		result.right_ind = _curb_point_index;
	}
	...
	else
	{
		...
		result.left_ind = _curb_point_index;
	}
...
int main(int argc, char** argv) 
{
    ...
    nh.getParam("debug_flag_0",_debug_flag);
    GlobalDebugData::setDebugTest0Flag(_debug_flag);
    nh.getParam("debug_flag_1",_debug_flag);
    GlobalDebugData::setDebugTest1Flag(_debug_flag);
    nh.getParam("debug_flag_2",_debug_flag);
    GlobalDebugData::setDebugTest2Flag(_debug_flag);
    nh.getParam("debug_flag_3",_debug_flag);
    GlobalDebugData::setDebugTest3Flag(_debug_flag);
    nh.getParam("debug_flag_left",_debug_flag);
    GlobalDebugData::setDebugTest4Flag(_debug_flag);
    nh.getParam("debug_flag_right",_debug_flag);
    GlobalDebugData::setDebugTest5Flag(_debug_flag);
    nh.getParam("process_layer", g_flag_process_ring);
    nh.getParam("simul_until", g_flag_simul_until);
    nh.getParam("debug_flag_6",_debug_flag);
    GlobalDebugData::setDebugTest6Flag(_debug_flag);
    ...
    
    {
		//char ts[128];
		//sprintf
        ROS_INFO("debug_flag_0 = %d\n",GlobalDebugData::getDebugTest0Flag());
        ROS_INFO("debug_flag_1 = %d\n",GlobalDebugData::getDebugTest1Flag());
        ROS_INFO("debug_flag_2 = %d\n",GlobalDebugData::getDebugTest2Flag());
        ROS_INFO("debug_flag_3 = %d\n",GlobalDebugData::getDebugTest3Flag());
        ROS_INFO("debug_flag_left  = %d\n",GlobalDebugData::getDebugTest4Flag());
        ROS_INFO("debug_flag_right = %d\n",GlobalDebugData::getDebugTest5Flag());
        ROS_INFO("process_layer = %d\n", g_flag_process_ring);
        ROS_INFO("simul_until = %f\n", g_flag_simul_until);
        ROS_INFO("debug_flag_6 = %d\n",GlobalDebugData::getDebugTest6Flag());
    }
